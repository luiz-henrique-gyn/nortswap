...
import { Box, Button, Flex, InjectedModalProps, LinkExternal, Message, Skeleton, Text } from '@allnext/uikit'
...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowBnbBalance, onDismiss }) => {
  ...
  const { balance: ntBalance, fetchStatus: ntFetchStatus } = useTokenBalance(tokens.nt.address)
  ...
  return (
    <>
      ...
      <Flex alignItems="center" justifyContent="space-between" mb="24px">
        <Text color="textSubtle">{t('NT Balance')}</Text>
        {ntFetchStatus !== FetchStatus.SUCCESS ? (
          <Skeleton height="22px" width="60px" />
        ) : (
          <Text>{getFullDisplayBalance(ntBalance, 18, 3)}</Text>
        )}
      </Flex>
